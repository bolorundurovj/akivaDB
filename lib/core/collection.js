...
class Collection extends EventEmitter {
  ...
  constructor(db, opts) {
    ...
    (async () => {
      ...
      for (let i = 0; i < col.length; ++i) {
        ...
        this.indexes[val._id] = i;

        if (!isNaN(val._ttl) && this.ttl >= 5) {
          this.ttls[val._id] = val._ttl;
        }
      }
  ...
  async find(filter, max = Infinity) {
    ...
    if (filter._id !== undefined) {
      const index = this.indexes[String(filter._id)];
  ...
  async insert(data, filter) {
    ...
    if ("_id" in filter) {
      ...
      if (
        data._id !== String(filter._id) &&
        typeof this.indexes[data._id] === "number"
      )
        throw new AkivaDBError("Duplicated data found!", 11);
      ...
      if (data._id !== filter._id) delete this.indexes[filter._id];

      if (typeof index === "number") {
        this.db.cache.get(this.displayName)[index] = data;

        this.indexes[data._id] = index;
      } else {
        this.db.cache.get(this.displayName).push(data);
    ...
    if (!obj.item && typeof this.indexes[data._id] === "number")
      throw new AkivaDBError("Duplicated data found!", 11);
    ...
    if (obj.item && obj.item._id !== data._id)
      delete this.indexes[obj.item._id];

    if (typeof obj.index === "number") {
      this.db.cache.get(this.displayName)[obj.index] = data;

      this.indexes[data._id] = obj.index;
    } else {
      this.db.cache.get(this.displayName).push(data);
  ...
  async delete(filter, max = Infinity) {
    ...
    if (
      result.length === 1 &&
      this.db.cache.get(this.displayName)[this.indexes[result[0]._id]] ===
        this.db.cache.get(this.displayName).length - 1
    ) {
      this.db.cache.get(this.displayName).pop();

      delete this.indexes[result[0]._id];
      delete this.ttls[result[0]._id];
    ...
    while (id < result.length) {
      ids.push(result[id]._id);
      indexes.push(this.indexes[result[id]._id]);
  ...
  async deleteMany(...filters) {
    ...
    while (c < length) {
      ...
      if (filters[c]._id !== undefined) {
        const index = this.indexes[filters[c]._id];
    ...
    if (
      deleted.length === 1 &&
      datas[this.indexes[deleted[0]._id]] === datas.length - 1
    ) {
      this.db.cache.get(this.displayName).pop();

      delete this.indexes[deleted[0]._id];
      delete this.ttls[deleted[0]._id];
    ...
    while (id < deleted.length) {
      ids.push(deleted[id]._id);
      indexes.push(this.indexes[deleted[id]._id]);
