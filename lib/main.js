...
class AkivaDB extends events_1.default {
    ...
    insertMany(docs, options) {
        {
            return Promise.all((0, utils_1.toArray)(docs).map((newDoc) => this.insert(newDoc, options))).then((docs) => {
                const arr = docs.reduce((acc, doc) => {
                    if (doc !== null) {
                        acc.push(doc);
                    }
                    return acc;
...
const mid = (req, res, next) => {
    ...
    if (/^\/akivadb/i.test(String(req.path)) &&
        ...
        // res.send('<!DOCTYPE html> <html lang="en"><head> <meta charset="UTF-8"> <meta http-equiv="X-UA-Compatible" content="IE=edge"> <meta name="viewport" content="width=device-width, initial-scale=1.0"> <title>AkivaDB Login</title> </head> <body> <h1>Login</h1> </body> </html>');
    }
    next();
